"""Memory-guarded launcher for A5 certificate recomputation."""
...
def main() -> None:
    values = yaml.safe_load((REPO / "configs/direction5_accr/a5_theory_lock.yaml").read_text("utf-8"))["resource_guard"]
    ...
    code = run_guarded(
        [sys.executable, str(REPO / "scripts/direction5_accr/run_a5_certificates.py")],
        cwd=REPO, environment=environment, limits=limits,
        monitor_log=REPO / "logs_accr/A5/MEMORY_MONITOR.jsonl",
        summary_path=REPO / "logs_accr/A5/MEMORY_MONITOR_SUMMARY.json",
    )
