"""Memory-guarded launcher for A6 development."""
...
def main() -> None:
    values = yaml.safe_load((REPO / "configs/direction5_accr/a6_validation_lock.yaml").read_text("utf-8"))["resource_guard"]
    ...
    wait_for_memory_preflight(
        limits,
        log_path=REPO / "logs_accr/A6/development/PRELAUNCH_MEMORY_WAIT.jsonl",
    )
    code = run_guarded(
        [sys.executable, str(REPO / "scripts/direction5_accr/run_a6_development.py")],
        cwd=REPO, environment=environment, limits=limits,
        monitor_log=REPO / "logs_accr/A6/development/MEMORY_MONITOR.jsonl",
        summary_path=REPO / "logs_accr/A6/development/MEMORY_MONITOR_SUMMARY.json",
    )
