"""Verify every payload file in an extracted Direction5 Phase-H package."""
...
def main() -> None:
    ...
    if expected != actual:
        raise SystemExit(
            f"manifest path mismatch missing={sorted(expected - actual)} extra={sorted(actual - expected)}"
        )
    for row in rows:
        path = ROOT / row["path"]
    ...
    print(f"DIRECTION5_PHASE_H_MANIFEST_OK files={len(rows)}")
